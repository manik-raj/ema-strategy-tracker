...
- **Retest Precision** — Configurable per tracking pair (default 0.4%), controls how close price must be to EMA to trigger a retest alert
- **Trend Change Alerts** — Notifies when price closes above/below the 21 EMA, reversing the current trend
- **EMA Retest Alerts** — Notifies when price pulls back within the configured retest precision % of the 21 EMA after a trend change
...
- **Alert 2 — EMA Retest**: fired once when price comes within the pair's retest precision % (default 0.4%) of the 21 EMA after a trend change
